...
export default async function AdminReservationsPage() {
  ...
  const totalPending = allReservations.filter(
    ...
    <main className="admin-shell">
      <header className="admin-header">
        <div>
          ...
          <h1>Réservations</h1>
          {totalPending > 0 && (
            <p className="res-pending-badge">{totalPending} demande{totalPending > 1 ? "s" : ""} en attente</p>
          )}
        </div>
        <div className="admin-actions">
          <a className="button button-secondary" href="/admin/reservations/historique">
            Historique
          </a>
